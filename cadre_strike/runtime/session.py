...
def default_automation_root() -> Path:
    ...
    env_raw = os.environ.get("CADRE_AUTOMATION_ROOT", "").strip()
    if env_raw:
        env = Path(env_raw)
        if env.is_dir():
            return env
    for parent in Path(__file__).resolve().parents:
        candidate = parent / "CADRE" / "attack-matrix" / "04-automation" / "linux"
        if candidate.is_dir():
            return candidate
    return Path.cwd()
...
def default_seed_path() -> Path | None:
    """Prefer CADRE lab seed (integration glue); never ship real lab secrets in RedStrike."""
    import os
    ...
    if env and Path(env).is_file():
        return Path(env)
    for parent in Path(__file__).resolve().parents:
        cadre_seed = parent / "CADRE" / "attack-matrix" / "Campaign" / "automation" / "lab-seed-creds.json"
        if cadre_seed.is_file():
            return cadre_seed
