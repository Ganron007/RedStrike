...
from cadre_strike.ad.parsers import parse_for_action
from cadre_strike.core.errors import GuardrailViolationError
from cadre_strike.core.models import ADRequest, EvidenceRecord, Finding, OperationResponse, RiskLevel
from cadre_strike.core.policy import ScopePolicy
from cadre_strike.core.runner import CommandRunner
...
class ActiveDirectoryAssessmentService:
    ...
    def asrep_roastable(self, request: ADRequest) -> OperationResponse:
        output_file = str(Path(gettempdir()) / f"cadre_asrep_{request.target.replace('.', '_')}.txt")
        return self._run(
            "asrep_roastable",
            "T1558.004",
            request,
    ...
    def kerberoastable(self, request: ADRequest) -> OperationResponse:
        output_file = str(Path(gettempdir()) / f"cadre_kerberoast_{request.target.replace('.', '_')}.txt")
        return self._run(
            "kerberoastable",
            "T1558.003",
            request,
